...
uint8_t state = 0;
#define NUM_STATES 2
#define STATE_OCTAVE_ARP 0
#define STATE_FM_PIANO 1
...
// this is the wave form of the oscillators [0:sin, 1:square, 2: pulse, 3:triangle, 4:sawtooth
uint8_t waveForm = 0;
uint8_t waveFM = 12;
...
void loop(void)
{
  prev_button_25 = button_25;
  button_25 = digitalRead(MODE_BUTTON);
  
  if(prev_button_25 != button_25){
    //button preseed or depressed
    if(button_25 == 0){
  ...
  if(state % NUM_STATES == STATE_OCTAVE_ARP){
  ...
  }else if(state % NUM_STATES == STATE_FM_PIANO){
    
     harmonicity = analogRead(0) >> 2;
      frequency[0] = ((miditof[key[0]]) * (pitchScale)) >> 12;
      frequency[1] = frequency[2] = frequency[3] = 0;
    if (key[0] == 0){
      gain = 0;
    }
    else
