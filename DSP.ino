...
ISR(TIMER2_COMPA_vect) {
  ...
  // calculate frequency mod  OctaveArp

  //state 0: octave arp
  if( state % NUM_STATES == STATE_OCTAVE_ARP)
  {
    //logic for the 4 keys
    sample = 0;

    
    for(uint8_t key =0; key < 4; key++){
     accumulator[key] += frequency[key];
      index[key] = accumulator[key] >> 8;
      
      oscil[key] = sineTable[index[key]];     
    }
    sample = (oscil[0] + oscil[1] + oscil[2] + oscil[3]) << 6;
  }else
  if( state % NUM_STATES == STATE_FM_PIANO){
    // calculate frequency mod piano FM
    phaseDeltaModulator = (frequency[0] * harmonicity) >> 6;   // calculate modulator frequency for a  given harmonicity
    phaseDeltaCarrier = frequency[0];                           // this is just pitch (although not in Hz)
    ...
    //carrier = waveTable[waveFM*256+ indexCarrier];triTable
    carrier = triTable[indexCarrier];
